...
# ── 1. Compute ratios (baseline timeout → ratio stored as NaN, bar = expensive) ──
def safe_ratio(numerator, denominator):
    ratios = []
    for n, d in zip(numerator, denominator):
        if d == TIMEOUT:
            ratios.append(np.nan)   # baseline timeout → mark as expensive
        else:
            ratios.append(n / d)
    return np.array(ratios)

ratio_p2_p1 = safe_ratio(phase2, phase1)
ratio_p3_p1 = safe_ratio(phase3, phase1)

# ── 3. Y-axis cap: hard 10× ratio ───────────────────────────────────────────
Y_MAX = 6.0
...
def classify(ratios, numerators):
    ...
    for r, n in zip(ratios, numerators):
        if np.isnan(r) or n == TIMEOUT:   # baseline timeout OR numerator timeout → expensive
...
plt.tight_layout()
plt.savefig("query_ratios_all.png", dpi=150, bbox_inches="tight")
print(f"Saved: query_ratios_all.png  |  Y-axis cap: {Y_MAX:.1f}×  |  Queries plotted: {len(qids)}")
plt.show()
