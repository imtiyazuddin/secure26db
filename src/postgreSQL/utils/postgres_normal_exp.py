...
for i in range(1, 23):
    q_id = str(i)
    if q_id not in tpch_queries:
        print(f"  [Q{i}] Skipped: Query not found in file.", flush=True)
        continue
        
    q_sql = tpch_queries[q_id]
    print(f"  -> [DEBUG] Firing Q{q_id}...", flush=True)
        
    avg_time = run_query_safe(q_sql, timeout_ms=3600000) 
    baseline_times[q_id] = avg_time
    
    if avg_time == float('inf'):
        print(f"  [Q{q_id}] Normal Query: TIMEOUT (> 3600s)\n", flush=True)
...
for filename in sorted(policy_queries.keys()):
    ...
    avg_time = run_query_safe(sql, timeout_ms=3600000) 
    
    if avg_time == float('inf'):
        print(f"  [{filename}] Normal Query: TIMEOUT (> 3600s)\n", flush=True)
    elif avg_time is None:
